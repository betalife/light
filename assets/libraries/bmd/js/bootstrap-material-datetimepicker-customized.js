(function ($, moment) {
  var matchColorClass = function (i, c) {
    var pattern = 'bmd-bg-';
    return (c.match(new RegExp('(^|\\s)' + pattern + '\\S+', 'g')) || []).join(' ');
  }
  ...
  function Plugin(element, options) {
    ...
    this.colorClass = 'bmd-bg-' + this.params.stateColor;
  ...
  Plugin.prototype =
    {
      ...
      initTemplate: function () {
        this.template = '<div class="dtp hidden" id="' + this.name + '">' +
          ...
          '<div class="dtp-time hidden ' + this.colorClass + '">' +
          ...
          '<button class="dtp-btn-clear btn bmd-flat-btn btn-' + this.params.stateColor + '">Clear</button>' +
          '<button class="dtp-btn-ok btn bmd-flat-btn pull-right btn-' + this.params.stateColor + '">' + this.params.okText + '</button>' +
          '<button class="dtp-btn-cancel btn bmd-flat-btn pull-right btn-' + this.params.stateColor + '">' + this.params.cancelText + '</button>' +
          '<div class="clearfix"></div>' +
          '</div>' +
          '</div>' +
          '</div>';
      ...
      initHours: function () {
        ...
        for (var h = 0; h < 12; ++h) {
          ...
          if (h == parseInt(cH))
            ...
            hourLink.addClass('selected ' + this.colorClass);
      ...
      initMinutes: function () {
        ...
        for (var m = 0; m < 60; m += 5) {
          ...
          if (m == 5 * Math.round(this.currentDate.minute() / 5)) {
            ...
            minuteLink.addClass('selected ' + this.colorClass);
      ...
      initHands: function (t) {
        ...
        this.$dtpElement.find('.dtp-hour-hand').css({
          ...
          // }).addClass((t === true) ? 'on' : '');
        }).addClass((t === true) ? 'on ' + this.colorClass : '');
        ...
        ({
          ...
          // }).addClass((t === false) ? 'on' : '');
        }).addClass((t === false) ? 'on ' + this.colorClass : '');
          ...
          if (calendar.days[i] != 0) {
            if (this.isBeforeMaxDate(moment(calendar.days[i]), false, false) === false || this.isAfterMinDate(moment(calendar.days[i]), false, false) === false) {
              ...
              if (moment(calendar.days[i]).locale(this.params.lang).format("DD") === moment(this.currentDate).locale(this.params.lang).format("DD")) {
                ...
                _template += '<a href="javascript:void(0);" class="dtp-select-day selected ' + this.colorClass + '">' + moment(calendar.days[i]).locale(this.params.lang).format("DD") + '</a>';
              } else {
      ...
      setElementValue: function () {
        ...
        this.$element.val(this.currentDate ?
          moment(this.currentDate).locale(this.params.lang).format(this.params.format) : null
        );
      ...
      _onSelectDate: function (e) {
        ...
        this.$dtpElement.find('a.dtp-select-day').removeClass('selected').removeClass(matchColorClass);
        $(e.currentTarget).addClass('selected ' + this.colorClass);
      ...
      _onSelectHour: function (e) {
        ...
        // $(e.currentTarget).addClass('selected');
        this.$dtpElement.find('a.dtp-select-hour').removeClass('selected').removeClass(matchColorClass);
        $(e.currentTarget).addClass('selected ' + this.colorClass);
      ...
      _onSelectMinute: function (e) {
        ...
        // $(e.currentTarget).addClass('selected');
        this.$dtpElement.find('a.dtp-select-minute').removeClass('selected').removeClass(matchColorClass);
        $(e.currentTarget).addClass('selected ' + this.colorClass);
      ...
      _onSelectAM: function (e) {
        ...
        // $(e.currentTarget).addClass('selected');
        $('.dtp-actual-meridien').find('a').removeClass('selected').removeClass(matchColorClass);
        $(e.currentTarget).addClass('selected ' + this.colorClass);
